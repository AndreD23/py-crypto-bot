...
# Tem que trocar o servidor de tempo do windows pra time.nist.gov.
# Deve ser feito pelo Painel de Controle > Data e Hora >  Horário na Internet
# Depois, ir em Configurações > Hora e Idioma > Data e hora, e clicar em Sincronizar agora

client = Client(api, secret, tld="com", testnet=True)


class Bot:
    def __init__(self, symbol, volume, no_of_safety_orders, proportion, profit_target):
        self.symbol = symbol
        self.volume = volume
        self.no_of_safety_orders = no_of_safety_orders
    ...
    def sell_all(self, symbol, df):
        volume = round(float(df['qty'].sum()), 3)
        ...
        while True:
            ...
            while True:
                ...
                if dev <= next_price_level * self.proportion:
                    if curr_no_of_safety_orders < self.no_of_safety_orders:
                        try:
